...
class Decoder(nn.Module):
    ...
    def setup(self) -> None:
        ...
        if self.decoder is not None:
            inv_phi = repr_class((*self.hidden_dims[:2], self.hidden_dims[-1]), activate_final=True,
                                 ensemble=self.ensemble)
            inv_phi = nn.Sequential([inv_phi, self.decoder()])
        else:
            inv_phi = repr_class((*self.hidden_dims, self.observation_dim), activate_final=False,
                                 ensemble=self.ensemble)

        self.inv_phi = inv_phi


    def __call__(self, phi, info=False):
        return self.inv_phi(phi)
...
class TempDATANetwork(nn.Module):
    ...
    def reconstruction(self, phi, **kwargs):
        return self.networks['decoder'](phi, **kwargs)

    def dynamic(self, skills, actions, **kwargs):
        return self.networks['dynamic'](jnp.concatenate([skills, actions], axis=-1), **kwargs)
    ...
    def __call__(self, observations, goals, actions, skills):
        ...
        rets = {
            'decoder': self.reconstruction(skills),
            'dynamic': self.dynamic(skills, actions),
